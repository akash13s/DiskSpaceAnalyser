'''
calculates space occupied by files
'''
# std lib
import os
import shutil
...
def calculate_size(start_path):
    ...
    for folder_path, folder_names, file_names in os.walk(start_path):
...
if os.path.exists(dirpath) is False:
    ...
    if os.listdir(path) == []:
        ...
        for i in range(1, 1000):
            print('''
Press 1 to go to child directory
Press 2 to go to parent directory
Press 3 to delete a subdirectory within current directory
Press 4 to delete a file within current directory
Press 5 to exit
            ''')
            # time.sleep(2)
